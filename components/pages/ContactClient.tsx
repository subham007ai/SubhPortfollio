...
export default function ContactClient() {
  ...
  return (
    <div className="mx-auto max-w-content px-6 md:px-10 pt-16 md:pt-24 pb-24">
      <div className="dot-matrix mb-4">Contact</div>
      ...
      <button onClick={copy} className="block w-full text-left group" aria-label={`Copy ${profile.email} to clipboard`}>
        ...
        <span className="dot-matrix mt-4 block" style={{ color: copied ? "var(--signal)" : "var(--muted)" }}>
...
function SocialLink({ label, href, handle }: { label: string; href: string; handle: string }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" className="group border-t hairline border-t-line pt-3">
      <span className="dot-matrix flex justify-between">
        <span>{label}</span>
        <span>↗</span>
      </span>
      <span className="mt-2 block font-display text-xl tracking-tight group-hover:underline underline-offset-4">
        {handle}
      </span>
    </a>
  );
